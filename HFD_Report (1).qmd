...
This report presents the development and evaluation of quantitative trading strategies using high-frequency data provided in the project.

The assets are divided into two groups. Group 1 includes SP and NQ futures observed at the 1-minute frequency. Group 2 includes CAD, AUD, XAU, and XAG assets observed at the 5-minute frequency.

Several candidate strategies were tested using in-sample data. The final strategy was selected based on performance stability, profitability after transaction costs, and robustness across quarters.

After selecting the final strategy, its performance was evaluated on out-of-sample quarters that were not used during the design phase. Performance is evaluated using metrics such as Sharpe ratio, cumulative P&L, and trading activity.

## Data and assumptions

### Data structure expected by the code

The dataset used in this project consists of parquet files provided in the assignment. 
The data is divided into two folders:

- data/ – contains the in-sample quarters used for strategy development.
- data_oos/ – contains the out-of-sample quarters used for evaluating the final strategy.

Group 1 includes SP and NQ futures observed at the 1-minute frequency.

Group 2 includes CAD, AUD, XAU and XAG observed at the 5-minute frequency.

Each parquet file contains synchronized intraday price series for the assets belonging to a specific group.
...
Before selecting the final trading strategy, two alternative approaches were considered.

The first approach was a trend-following strategy based on moving-average crossovers. 
This method attempts to capture persistent price movements by comparing short-term 
and long-term moving averages of log-prices. When the short-term moving average 
exceeds the long-term average, the strategy opens a long position. When the short-term 
average falls below the long-term average, the strategy opens a short position.

The second approach was a mean-reversion strategy. This approach assumes that short-term 
price deviations from a reference level tend to revert back to the mean, and therefore 
opens positions against large temporary deviations.

After testing both approaches using the in-sample data, the moving-average crossover 
strategy was selected as the final strategy because it produced more stable performance 
and was less sensitive to transaction costs.
...
def load_quarter_group_files_old(data_root: Path):
...
def load_quarter_group_files(data_root: Path):
    ...
    for path in sorted(data_root.glob("*.parquet")):
        ...
        data_tag, year, quarter_tag = parts

        if data_tag not in {"data1", "data2"}:
            continue

        if not quarter_tag.startswith("Q"):
            continue

        quarter = f"{year}{quarter_tag}"
        group = "group1" if data_tag == "data1" else "group2"

        df = pd.read_parquet(path).copy()

        dt_candidates = [c for c in df.columns if c.lower() in {"time", "date", "datetime", "timestamp"}]
        ...
        assets = GROUP1_ASSETS if group == "group1" else GROUP2_ASSETS
        missing = [a for a in assets if a not in df.columns]
        if missing:
            raise ValueError(f"Missing asset columns {missing} in {path.name}")

        df = df[assets].apply(pd.to_numeric, errors="coerce").dropna(how="all")

        out.setdefault(quarter, {})[group] = df

    if not out:
        raise ValueError(f"No parquet files detected in {data_root}")
...
### Strategy-level summary tables

The following tables summarize the final trading strategy at the group level. For each quarter and sample split, daily P&L is aggregated across all assets in the group and performance statistics are computed for the strategy as a whole.

#### Group 1 strategy summary

```{python}
from IPython.display import display

strategy_summary_path = RESULTS_DIR / "strategy_summary_table.csv"

def summarize_strategy_tables(data_root: Path, sample_label: str) -> pd.DataFrame:
    ...
    for quarter, gmap in quarter_group_data.items():
        for group, df in gmap.items():
            ...
            for asset in df.columns:
                ...
                tmp = daily.copy()
                tmp.columns = [f"{c}_{asset}" for c in tmp.columns]
                parts.append(tmp)

            if not parts:
                continue

            combined = pd.concat(parts, axis=1).fillna(0.0)

            gross_daily = combined.filter(like="gross_daily_").sum(axis=1)
            net_daily = combined.filter(like="net_daily_").sum(axis=1)
            ntrades_daily = combined.filter(like="ntrades_daily_").sum(axis=1)
            ...
            rows.append({
                ...
                "grossSR": _safe_sharpe(gross_daily),
                "netSR": _safe_sharpe(net_daily),
                "grossCumPnL": float(gross_daily.sum()),
                "netCumPnL": float(net_daily.sum()),
                "av_ntrades": float(ntrades_daily.mean()),
            })
...
if strategy_summary_path.exists():
    ...
    strategy_summary = pd.concat(
        [
            summarize_strategy_tables(DATA_ROOT, "in-sample"),
            summarize_strategy_tables(DATA_ROOT_OOS, "out-of-sample"),
        ],
        ignore_index=True
    )
    strategy_summary = strategy_summary.sort_values(["group", "sample", "quarter"]).reset_index(drop=True)
    strategy_summary.to_csv(strategy_summary_path, index=False)

g1_table = strategy_summary[strategy_summary["group"] == "group1"].copy()

display(g1_table)
```

#### Group 2 strategy summary
```{python}
g2_table = strategy_summary[strategy_summary["group"] == "group2"].copy()
display(g2_table)
```
